...
CANTONESE_REPLACEMENTS = {
    ...
    "嘛": "",         # 語氣詞，刪掉通常無害
    # M6-1b：移除以下容易誤傷的項：
    # "係": "是",     # 移除：會誤傷「關係、係數、聯係、體係」等
    # "呀": "",       # 移除：「是呀、好呀」是正常國語語氣詞
    # "啦": "",       # 移除：「好啦、算啦」是正常國語語氣詞
}

# M6-1b：白名單保護常見詞，對「係」做安全上下文替換
PROTECTED_WORDS = {"關係", "係數", "聯係", "維係", "體係", "係數"}

def _remove_cantonese(text: str) -> str:
    """輕量粵語詞彙後處理，替換常見粵語詞為國語。
    M6-1b：移除誤傷項，對「係」只在安全上下文時替換。"""
    import re
    ...
    # M6-1b：對「係」做上下文感知替換
    # 只有當「係」不在白名單詞中、且前後是合適的字時才替換
    # 寧可不換也不誤傷，因為 system prompt 已在源頭禁粵語
    for protected in PROTECTED_WORDS:
        if "係" in protected and protected in text:
            # 白名單詞已出現，不替換任何「係」（保守策略）
            return text.strip()

    # 如果沒有保護詞，用簡單正則：「係」前後不是中文時才替換（誤傷風險仍大，所以保留）
    # 更安全的做法：直接不替換「係」，讓 system prompt 在源頭控制
    # 因為這是後處理保險，不是主要防線
