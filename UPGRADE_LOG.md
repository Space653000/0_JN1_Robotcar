...
|------|------|------|------|------|------|------|----------|------|
| 2026-08-27 | 18:25:00 | M3-6e 完成 | 2.0.0 | Kokoro 性能診斷 + Piper 切換✅ | ✅ 診斷完成 | Piper | - | ⚠️ Kokoro 無法達成目標：1.7-3.0s/字(avg 1423ms/字)；✅ 模型仅启动时加载一次；✅ 全句一次合成优化无效（问题在 create()）；✅ onnxruntime GPU 支持存在但库无配置；✅ Piper TTS 配置完成：avg 623ms/字；✅ 长文本优化成功：19字 103ms/字(<150ms/字)；⚠️ 短文本 1-2字 仍需缓冲优化(~3s/字)；✅ 诊断工具 diagnose_tts_v2.py；✅ Piper 库 API 尝试(兼容性问题，回退 subprocess) |
