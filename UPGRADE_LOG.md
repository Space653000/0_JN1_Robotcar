...
|------|------|------|------|------|------|------|----------|------|
| 2026-08-27 | 18:20:00 | M3-6e 完成 | 2.0.0 | Kokoro 性能診斷 + Piper 切換 ✅ | ✅ 診斷完成 | Piper | - | ⚠️ Kokoro 合成速度 1.7-3.0s/字（平均1423ms/字），无法达成 <1s 目标；✅ 模型确认仅启动时加载一次，无重复加载；✅ 分段优化改为全句一次合成（无性能改善，说明问题在模型本身）；✅ onnxruntime GPU 支持存在但 kokoro_onnx 库不暴露配置接口；✅ 已切换到 Piper TTS（zh_CN-huayan）；✅ Piper 性能：122-1748ms/字（平均641ms/字），长文本(>10字)可接近目标，短文本因固定开销(~1700ms)偏慢；✅ 诊断脚本 diagnose_tts_v2.py 已上传；✅ Fallback 脚本已就绪 |
